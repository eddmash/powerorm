...
class BaseOrm extends BaseObject
{
    ...
    {
        $path = null;
        if (self::getInstance()->getSettings()->getModelsPath()):
            $path = realpath(self::getInstance()->getSettings()->getModelsPath());
            if (!$path && !file_exists(self::getInstance()->getSettings()->getModelsPath())):
                throw new FileHandlerException(
                    sprintf(
                        "The path '%s' does not exist please check the path is correct",
                        self::getInstance()->getSettings()->getModelsPath()
                    )
                );
            endif;
        endif;

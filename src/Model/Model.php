...
abstract class Model extends DeconstructableObject implements ModelInterface, ArrayObjectInterface, JsonSerializable
{
    ...
    public function __construct($kwargs = [])
    {
        ...
        $this->loadMeta(ArrayHelper::getValue($kwargs, 'registry', null));
//        Signal::dispatch(Signal::MODEL_PRE_INIT, $this, $kwargs);
    ...
    private function loadMeta(Registry $registry = null)
    {
        ...
        try {
            ...
            $model = $registry->getRegisteredModel($name);
...
//            static::setupClassInfo(null, ['registry' => $registry]);
        }
    }
...
//        }
        ...
        try {
            $registry = ArrayHelper::getValue($kwargs, 'registry',
                ArrayHelper::STRICT);
        } catch (KeyError $e) {
            throw new InvalidArgumentException('registry not provided');
        }
        ...
        if ($registry->hasModel(get_class($this)) && $registry->ready) {
            ...
            $metaSettings['registry'] = $registry;
    ...
    public static function objects(Registry $registry = null)
    {
        ...
        $modelInstance = static::createObject(['registry' => $registry]);
